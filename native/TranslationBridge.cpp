...
struct TranslationEngine {
    ...
    explicit TranslationEngine(const std::string &directory)
        ...
        if (!source.Load(directory + "/source.spm").ok() || !target.Load(directory + "/target.spm").ok())
...
char *translation_run(void *handle, const char *text, int *error) {
    *error = 2;
    try {
        ...
        if (!engine.source.Encode(text, &tokens).ok() || tokens.empty()) return nullptr;
        ...
        std::string output;
        if (!engine.target.Decode(results.front().hypotheses.front(), &output).ok() || output.empty()) return nullptr;
        *error = 0;
